...
func findMatch(sum int, sums map[int]int, nums []int) int {
	foundMatch := false
	for _, n := range nums {
		sum += n
		if _, ok := sums[sum]; ok {
...
func main() {
	...
	sums[0] = 0
	fmt.Printf("Day 01, Part 2: %v", findMatch(0, sums, freqs))
	fmt.Println()
}
